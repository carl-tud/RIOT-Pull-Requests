...
static bool rx_data_waiting = false;
static bool tx_data_finished = false;
...
static uint8_t data_length_rx = 0;
...
/* static void process_write_command(uint8_t block_no, const uint8_t *bytes)
{
    ...
    send_ack_nack(true);
    int_bytes_as_hex(tag->memory, tag->memory_size);
} */
...
static void irq_event_handler(const nrfx_nfct_evt_t *event)
{
    ...
    if (event->evt_id == NRFX_NFCT_EVT_RX_FRAMEEND) {
        ...
        memcpy(data_buffer_rx, event->params.rx_frameend.rx_data.p_data,
               data_length_rx);
        rx_data_waiting = true;
    }
    else if (event->evt_id == NRFX_NFCT_EVT_TX_FRAMEEND) {
        LOG_DEBUG("End of transmission\n");
        tx_data_finished = true;
    }
...
int listen_a(nfcdev_t *nfcdev, const nfc_a_listener_config_t *config) {
    ...
    configure_fdt(0x80000, 0x0480);

    nrfx_nfct_enable();

    while (!selected) {
        // LOG_DEBUG("Waiting for selection...\n");
        ztimer_sleep(ZTIMER_MSEC, 1);
    }

    while (!rx_data_waiting) {
        ztimer_sleep(ZTIMER_MSEC, 1);
    }
